...
<template>
   <section id="hero" class="relative min-h-screen flex items-start overflow-hidden">
        ...
        <div class="absolute top-0 left-0 w-[700px] h-[700px] rounded-full pointer-events-none"
             style="background:radial-gradient(circle, rgba(0,119,182,0.15) 0%, transparent 70%)"></div>
        <div class="absolute bottom-0 right-0 w-[600px] h-[600px] rounded-full pointer-events-none"
             style="background:radial-gradient(circle, rgba(2,62,138,0.12) 0%, transparent 70%)"></div>

        <!-- ── CONTENT — full width container ── -->
        <div class="relative z-10 w-full px-8 lg:px-16 xl:px-24 pt-20 pb-12
                    grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-12 items-center">
            ...
            <div>
                ...
                <p class="text-[#7ab8cc] leading-relaxed mb-6 max-w-lg" style="font-size:1rem">
            ...
            <div class="hidden lg:block">
                <div class="glass-card p-6 ocean-glow">
                    <div class="flex items-center justify-between mb-5">
                        ...
                        <span class="font-mono text-[0.65rem] text-[#0a2347] tracking-widest">SYSTEM ONLINE</span>
                        <span class="font-mono text-[0.65rem] text-[#00b4d8] bg-[rgba(0,180,216,0.1)] px-2.5 py-1 rounded-lg">● ACTIVE</span>
                    </div>

                    <div class="space-y-3 mb-5">
                        <div v-for="(row, i) in vizRows" :key="row.name"
                             class="bg-[rgba(0,180,216,0.04)] border border-[rgba(0,180,216,0.1)] rounded-xl p-3.5 flex items-start gap-3">
                            <div class="w-9 h-9 rounded-lg bg-[rgba(0,180,216,0.1)] border border-[rgba(0,180,216,0.15)]
                                        flex items-center justify-center font-mono text-[0.75rem] text-[#00b4d8] shrink-0">
                                {{ row.icon }}
                            </div>
                            <div class="flex-1 min-w-0">
                                <div class="font-semibold text-[0.85rem] text-[#caf0f8] mb-0.5">{{ row.name }}</div>
                                <div class="font-mono text-[0.68rem] text-[#0a2347] mb-2">{{ row.sub }}</div>
                                <div class="h-1.5 bg-[rgba(0,180,216,0.08)] rounded-full overflow-hidden">
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <div v-for="s in [['20+','Projects'],['5+','Yrs Exp'],['10+','CTF Wins']]" :key="s[1]"
                             class="bg-[rgba(0,180,216,0.04)] border border-[rgba(0,180,216,0.1)] rounded-xl p-3 text-center">
                            <div class="font-extrabold text-[#00b4d8] text-2xl leading-none">{{ s[0] }}</div>
                            <div class="font-mono text-[0.6rem] text-[#0a2347] mt-1 tracking-wide">{{ s[1] }}</div>
                        </div>
                    </div>
                </div>
            </div>
...
.social-pill:hover { border-color: #00b4d8; color: #00b4d8; }
</style>
