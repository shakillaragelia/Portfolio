...
<template>
    <section id="about" class="py-20 relative">
        ...
        <div class="w-full px-8 lg:px-16 xl:px-24">
            <div class="section-label mb-3">02 / About</div>
            <h2 class="font-extrabold tracking-tight mb-12 leading-none"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                ...
                <div class="flex flex-col items-center gap-6">
                    <div class="relative">
                        ...
                        <div class="w-56 h-56 rounded-full overflow-hidden border-2 border-[rgba(0,180,216,0.3)]"
                             style="box-shadow: 0 0 0 8px rgba(0,180,216,0.06), 0 0 50px rgba(0,180,216,0.15)">
                            <img v-if="settings?.photo" :src="settings.photo" alt="Photo" class="w-full h-full object-cover" />
                            <div v-else class="w-full h-full bg-[#041226] flex items-center justify-center text-7xl">👨‍💻</div>
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-3 w-full max-w-sm">
                        <div v-for="card in infoCards" :key="card.label" class="glass-card px-4 py-3">
                            <div class="font-mono text-[0.58rem] text-[#0a2347] tracking-widest uppercase mb-1">{{ card.label }}</div>
                            <div class="font-bold text-sm" :class="card.accent ? 'text-[#00b4d8]' : 'text-[#caf0f8]'">{{ card.value }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="font-extrabold tracking-tight mb-5 leading-tight"
                         style="font-size: clamp(1.6rem, 3vw, 2.5rem)">
                        {{ settings?.name ?? 'Your Name' }}
                    </div>
                    <p class="text-[#7ab8cc] leading-relaxed mb-4" style="font-size:0.97rem">
                        A <strong class="text-[#caf0f8] font-semibold">full-stack web developer</strong> passionate about
                        building robust, secure, and scalable web applications. Core stack:
                        <strong class="text-[#caf0f8] font-semibold">PHP & Laravel</strong> untuk backend,
                        <strong class="text-[#caf0f8] font-semibold">Filament</strong> untuk admin panel,
                        dan <strong class="text-[#caf0f8] font-semibold">Vue.js</strong> untuk frontend modern.
                    </p>
                    <p class="text-[#7ab8cc] leading-relaxed mb-6" style="font-size:0.97rem">
                        ...
                        web application penetration testing. Aktif ikut CTF competition dan bug bounty.
                    </p>
                    ...
                    <div class="flex gap-3 flex-wrap">
                        <a v-if="settings?.cv_url" :href="settings.cv_url" target="_blank" class="btn-ocean">📄 Download CV</a>
                        <a href="#projects" class="btn-ghost"
                           @click.prevent="document.getElementById('projects')?.scrollIntoView({behavior:'smooth'})">&lt;/&gt; View Projects</a>
                    </div>
                </div>
            </div>
        </div>
