                ...
                <div class="flex flex-col items-center gap-6" data-reveal>
                    ...
                    <div class="grid grid-cols-2 gap-2.5 w-full max-w-xs">
                        <div v-for="card in infoCards" :key="card.label"
                            ...
                            <div class="font-bold text-sm text-[#caf0f8]" :class="card.accent ? 'text-[#00b4d8]' : ''">{{ card.value }}</div>
                        </div>
                    </div>
                </div>
...
export default {
    computed: {
        infoCards() {
            return [
                { label: 'Based in',   value: this.settings?.location  ?? 'Indonesia 🇮🇩' },
                { label: 'Experience', value: (this.settings?.years_exp ?? '5') + '+ Years' },
                { label: 'Status',     value: this.settings?.available === 'true' ? '● Available' : '○ Busy', accent: true },
